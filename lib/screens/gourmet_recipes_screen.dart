...
import 'package:my_diet/data/gourmet/gourmet_recipes_data.dart';
import 'package:my_diet/data/methodology_registry.dart';
import 'package:my_diet/screens/gourmet_recipe_detail_screen.dart';
import 'package:my_diet/widgets/common_widgets.dart';
import 'package:my_diet/widgets/methodology_recipes_list.dart';

/// Список рецептов методики «Диета для гурманов» по этапам и типам блюд.
class GourmetRecipesScreen extends StatelessWidget {
  const GourmetRecipesScreen({super.key});
  ...
  static List<MethodologyRecipeStageGroup> _stageGroups(
    ...
    void Function(GourmetRecipe recipe) onTap,
  ) {
    return [
          ...
          subcategories: [
            for (final def in _subcategoryDefs)
              MethodologyRecipeSubcategoryGroup(
                ...
                title: def.title,
                recipes: [
                  for (final recipe in GourmetRecipesData.forStageType(
                    stageIndex,
                    salads: def.salads,
                  ))
              ...
              child: MethodologyRecipesStageAccordion(
                ...
                stages: _stageGroups(
                  ...
                  (recipe) => Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) =>
                          GourmetRecipeDetailScreen(recipe: recipe),
                    ),
                  ),
                ),
              ),
