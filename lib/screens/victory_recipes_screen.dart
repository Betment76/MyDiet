...
import 'package:my_diet/data/victory/victory_recipes_data.dart';
import 'package:my_diet/screens/victory_recipe_detail_screen.dart';
import 'package:my_diet/widgets/common_widgets.dart';
import 'package:my_diet/widgets/methodology_recipes_list.dart';

/// Список рецептов «Диета трудная» по этапам и типам блюд.
class VictoryRecipesScreen extends StatelessWidget {
  const VictoryRecipesScreen({super.key});
  ...
  static List<MethodologyRecipeStageGroup> _stageGroups(
    ...
    void Function(VictoryRecipe recipe) onTap,
  ) {
    return [
          ...
          subcategories: [
            for (final def in _subcategoryDefs)
              MethodologyRecipeSubcategoryGroup(
                ...
                title: def.title,
                recipes: [
                  for (final recipe in VictoryRecipesData.forStageType(
                    stageIndex,
                    salads: def.salads,
                  ))
              ...
              child: MethodologyRecipesStageAccordion(
                ...
                stages: _stageGroups(
                  ...
                  (recipe) => Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) =>
                          VictoryRecipeDetailScreen(recipe: recipe),
                    ),
                  ),
                ),
              ),
